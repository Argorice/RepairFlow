...
/**
 * По умолчанию API живёт на том же origin, что и приложение: в разработке это делает прокси
 * Vite, в продакшне — rewrite на Vercel. Так кука с refresh-токеном остаётся first-party,
 * а браузеры всё агрессивнее режут сторонние куки — с прямым обращением к другому домену
 * сессия жила бы до первой перезагрузки страницы.
 *
 * VITE_API_URL оставлен как аварийный выход: указать прямой адрес бэкенда всё ещё можно.
 * Хвостовой слэш срезается — иначе получится «…com//health».
 */
export const apiBaseUrl = (import.meta.env.VITE_API_URL ?? '').replace(/\/+$/, '')

/**
 * Хабу SignalR прокси не нужен: он не полагается на куку — токен уезжает query-параметром.
 * Зато прямой адрес даёт настоящий WebSocket: через rewrite на Vercel апгрейд соединения
 * не проходит, и SignalR сползает на long polling. Если адрес не задан, хаб живёт на том же
 * origin, что и API, — работать будет, просто менее эффективно.
 */
export const realtimeBaseUrl =
  (import.meta.env.VITE_REALTIME_URL ?? '').replace(/\/+$/, '') || apiBaseUrl
